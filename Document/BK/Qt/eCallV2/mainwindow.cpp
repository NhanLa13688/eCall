    ...
    , temp(27) // Khoi tao nhiet do ban dau la 27
{
    ...
    background->setPixmap(QPixmap(":/new/prefix1/odo.jpg"));
    background->setScaledContents(true); // Co giãn ảnh cho phù hợp kích thước
    background->resize(this->size()); // Đảm bảo QLabel bao phủ toàn bộ cửa sổ
    background->setGeometry(this->rect());
    background->lower(); // Đặt QLabel xuống dưới cùng
...
void MainWindow::handleSOS()
{
        ...
        QTcpSocket socket; // Sử dụng QTcpSocket cho giao tiếp TCP
            socket.connectToHost("192.168.137.88", 65432); // Địa chỉ IP và cổng của server
